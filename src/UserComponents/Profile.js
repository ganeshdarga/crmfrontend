...
const Profile = () => {
  ...
  const viewProfile=()=>{
    Axios.get(`https://crmbackend-1-9mff.onrender.com/api/v1/user/viewUser/?userEmail=${userEmail}`).then((response)=>{
  ...
  const submiteditdata = (id)=>{
    ...
    if (userName === "" || userEmail_id === "" || useAlterEmail === "" || userMobile === "" || UserGender === "" || userAddress === ""){
      ...
      Axios.post(`https://crmbackend-1-9mff.onrender.com/api/v1/user/setuserEdit/?userEmail=${userEmail}`,{
      userName:userName,
