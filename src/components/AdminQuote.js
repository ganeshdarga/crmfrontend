...
const AdminQuote = () => {
  ...
  const getQuotes = () => {
    Axios.get("https://crmbackend-67.onrender.com/api/v1/prequest/", {}).then((res) => {
...
//     alert("No data found");
// });
    Axios.get(`https://crmbackend-67.onrender.com/api/v1/prequest/getiddata/?id=${id}`,{
    }).then((res)=>{
      setCustomdata(res.data)
    }).catch((err)=>{
      alert(err)
  ...
  const submitbtn=(id,email)=>{
    ...
    if (statusquote === "" || adminremark === ""){
      ...
      Axios.post(`https://crmbackend-67.onrender.com/api/v1/submitcustomdata/${id}`,{
      status1:status1,
    ...
    Axios.post(`https://crmbackend-67.onrender.com/api/v1/sendnotification/${email}`,{
